...
class MicrocystDataset(Dataset):
    ...
    def _sampleCandidator(self,label, mask):
        ...
        if rows.size > 0:
            ...
            candidate_rows = np.arange(startrow, max(startrow+int(window_size/4)+1,endrow), int(window_size/4))
            candidate_cols = np.arange(startcol, endcol, int(window_size/4))
    ...
    def __init__(self, image_paths, mask_paths, label_paths, window_size, valid_surface_threshold = 0.5, num_cores=8, balance_dataset=True, connected_component_size_limit=1024, preload_dataset = True):
        ...
        if preload_dataset:
            self.current_samples = [(row, col, label, current_image_index, self._extractWindow(self.current_images[current_image_index], (row,col))) for (row, col, label, current_image_index) in self.current_samples]
    ...
    def __getitem__(self,idx):
        ...
        if not self.preload_dataset:
            ...
            row, col, label, current_image_index, sample = self.current_samples[idx]
